    ...
    <n-card :style="{ width: '70%', height: store.PageHeight * 0.9 + 'px' }" title="添加城市" :bordered="true" size="huge">
      <n-form ref="formRef" label-placement="left" label-align="left" :model="state.createCity">
        <n-grid :cols="24" :x-gap="24">
          ...
          <n-form-item-gi label="传送门" :span="24">
            <n-dynamic-tags v-model:value="state.createCity.portal" />
          </n-form-item-gi>
    ...
    <template #default>
      <n-grid :cols="5" :x-gap="12" :y-gap="8">
        <n-gi v-for="i in state.getcities?.records" :key="i._id">
          <n-thing style="border: 1px dashed black; padding: 10px; text-align: left">
            <template #header>城市：{{ i.cityName }}-lv.{{ i.level }}</template>
            <template #header-extra>
              <n-popover trigger="hover" :keep-alive-on-hover="false" placement="bottom">
                <template #trigger>
                  <n-icon v-if="i.isCapital" size="30">
                    <fuben />
                  </n-icon>
                </template>
                <span>副本</span>
            ...
            <template #action>
              <n-space justify="end">
                <n-button size="tiny">NPC</n-button>
                <n-button size="tiny">怪物</n-button>
                <n-button size="tiny">Buff</n-button>
                <n-button size="tiny">传送门</n-button>
              </n-space>
            </template>
          </n-thing>
        </n-gi>
